#!/usr/bin/env python3
"""Validate that an optimized transcript preserves the raw transcript content."""
...
def extract_transcript_body(text: str) -> tuple[str, int, bool]:
...
def main() -> int:
    ...
    optimized_text = read_text(args.optimized)

    transcript_body, section_count, found_summary = extract_transcript_body(optimized_text)
    raw_normalized = normalize(raw_text)
    optimized_normalized = normalize(transcript_body)

    errors: list[str] = []

    if args.require_sections and section_count == 0:
        errors.append("No section subtitles matching '### Section N: <subtitle>' were found.")

    if args.require_summary and not found_summary:
    ...
    print(f"Summary found: {'yes' if found_summary else 'no'}")
    print(f"Normalized transcript length: {len(optimized_normalized)} characters")
    return 0

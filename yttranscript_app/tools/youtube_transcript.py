...
DEFAULT_MAX_TRACK_BYTES = 10 * 1024 * 1024
...
class TranscriptBuildResult:
    """Normalized transcript build output for pipeline use."""
...
def _resolve_configured_binary(env_var: str) -> list[str] | None:
    """Use an explicitly configured binary path when present."""
    ...
    raise TranscriptError(
        f"{env_var} points to `{configured}`, but that executable could not be found."
    )


def detect_yt_dlp_runner() -> list[str]:
    """Resolve yt-dlp from either the CLI or the installed Python module."""
    configured = _resolve_configured_binary("YTTRANSCRIPT_YT_DLP_BIN")
    ...
    raise TranscriptError(
        "yt-dlp is required for YouTube metadata and download support, but it is not installed. "
        "Install project dependencies with `pip install -e .` or install it directly with "
        "`pip install yt-dlp`."
    )


def extract_youtube_video_id(url: str) -> str:
    """Extract a supported YouTube video id from a user-supplied URL."""
    parsed = urlparse(url.strip())
    if parsed.scheme not in {"http", "https"}:
        raise TranscriptError("Only http(s) YouTube URLs are supported.")

    host = parsed.netloc.lower()
    if host not in YOUTUBE_HOSTS:
        raise TranscriptError("Only direct YouTube URLs are supported.")
    ...
    if not re.fullmatch(r"[A-Za-z0-9_-]{11}", video_id):
        raise TranscriptError("Could not determine a valid YouTube video id from the provided URL.")
    return video_id


def canonicalize_youtube_url(url: str) -> str:
    """Normalize supported YouTube input URLs into a stable watch URL."""
...
def load_metadata(url: str, *, timeout_seconds: int = DEFAULT_COMMAND_TIMEOUT_SECONDS) -> dict:
    runner = detect_yt_dlp_runner()
    completed = run_command(
        runner + ["--dump-single-json", "--skip-download", "--no-warnings", url],
...
def download_track(
    ...
    *,
    ...
    max_bytes: int = DEFAULT_MAX_TRACK_BYTES,
) -> Path:
            ...
            with path.open("wb") as handle:
                while True:
                    ...
                    if written > max_bytes:
                        raise TranscriptError("Subtitle track exceeded the maximum allowed size.")
                    handle.write(chunk)
    except TimeoutError as exc:
        raise TranscriptError("Subtitle track download timed out.") from exc
    except OSError as exc:
        raise TranscriptError(f"Could not download subtitle track: {exc}") from exc
    return path
...
def coverage_report(segments: Sequence[Segment], duration: float) -> dict[str, float]:
    if not segments:
        return {
            "coverage": 0.0,
...
def transcript_is_complete(
    ...
    *,
    ...
    return (
        ...
        and report["first_gap"] <= max_edge_gap
        and report["last_gap"] <= max_edge_gap
        and report["max_internal_gap"] <= max_internal_gap
    )


def describe_report(report: dict[str, float]) -> str:
    return (
...
def download_audio(
    ...
    *,
    ...
    run_command(
        runner + ["-f", "bestaudio/best", "-o", output_template, "--no-warnings", url],
        capture_output=True,
        timeout_seconds=timeout_seconds,
    )
    candidates = [
...
def detect_whisper_runner() -> List[str]:
    configured = _resolve_configured_binary("YTTRANSCRIPT_WHISPER_BIN")
    if configured is not None:
        return configured
...
def generate_transcript(
    ...
    *,
    ...
    max_edge_gap: float = DEFAULT_MAX_EDGE_GAP,
    max_internal_gap: float = DEFAULT_MAX_INTERNAL_GAP,
    max_chars: int = 84,
    command_timeout_seconds: int = DEFAULT_COMMAND_TIMEOUT_SECONDS,
    download_timeout_seconds: int = DEFAULT_DOWNLOAD_TIMEOUT_SECONDS,
    max_track_bytes: int = DEFAULT_MAX_TRACK_BYTES,
) -> TranscriptBuildResult:
    """Build and save a validated transcript artifact for a YouTube URL."""
    canonical_url = canonicalize_youtube_url(url)
    metadata = load_metadata(canonical_url, timeout_seconds=command_timeout_seconds)
    duration = float(metadata.get("duration") or 0.0)
    chosen_language = lang or metadata.get("language") or metadata.get("original_language")
    ...
    with tempfile.TemporaryDirectory(prefix="yttranscript-") as temp_root:
        ...
        if not force_asr:
            track = choose_track(metadata, lang)
            if track is not None:
                ...
                transcript_segments = normalize_segments(parse_track(track_path), max_chars)
        ...
        if not transcript_segments:
            ...
            source_label = f"whisper ASR ({whisper_model})"
            report = coverage_report(transcript_segments, duration)
    ...
    return TranscriptBuildResult(
        ...
        report=report,
        source_label=source_label,
        language=chosen_language or "unknown",
        content=content,
...
def main() -> int:
    ...
    try:
        output_path = Path(args.output) if args.output else default_output_path(load_metadata(canonicalize_youtube_url(args.url)))
        ...
        print(f"Wrote transcript to {result.output_path}")
        print(describe_report(result.report))
        if args.stdout:
            print()
